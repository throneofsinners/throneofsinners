...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [{ title: "Sanctuary Dashboard" }, { name: "robots", content: "noindex" }],
  }),
  component: Dashboard,
});
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={signOut}
          className="rounded-md border border-border bg-secondary px-3 py-2 text-sm text-ivory hover:bg-gold/10"
        >
          Sign out
        </button>
      </div>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Tile
          ...
          body="Review confessions and prayer requests. Triage by status and risk."
        />
        <Tile
          to="/chambers"
          title="Peer Chambers"
          body="Restoration circles where members walk together under a steward."
        />
        {isAdmin && (
          <Tile
            to="/admin/invites"
            title="Invite Pastors"
            body="Send invitations. Grant roles. Manage shepherds."
          />
        )}
        {isAdmin && (
          <Tile
            to="/admin/audit"
            title="Audit Log"
            body="Every pastoral action, immutable and timestamped."
          />
        )}
      </div>
    </main>
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div
      className={`rounded-lg border ${accent ? "border-red-500/40 bg-red-950/30" : "border-border bg-secondary/40"} p-5`}
    >
      <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">{label}</p>
      <p className={`mt-2 font-serif text-4xl ${accent ? "text-red-300" : "text-ivory"}`}>
        {value}
      </p>
    </div>
  );
...
function Tile({ to, title, body }: { to: string; title: string; body: string }) {
  return (
    <Link
      to={to}
      className="block rounded-lg border border-border bg-secondary/30 p-5 transition-colors hover:border-gold/40 hover:bg-gold/5"
    >
