...
function Inbox() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => listFn({ data: { status, type, risk_only: riskOnly, q } }),
  });
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="mt-6 flex flex-wrap items-end gap-3 rounded-lg border border-border bg-secondary/30 p-4">
        ...
        <Field label="Type">
          <select value={type} onChange={(e) => setType(e.target.value as never)} className="rounded-md border border-border bg-background px-2 py-1.5 text-sm text-ivory">
