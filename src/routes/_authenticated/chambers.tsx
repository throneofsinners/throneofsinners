  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Small circles where members walk together. Anonymous to one another, watched by stewards.
      </p>
      ...
      {isPastoral && (
        ...
        >
          <input
            ...
            placeholder="Topic (e.g. Healing from addiction)"
          ...
          />
          <button
            className="sm:col-span-4 rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground hover:opacity-90 disabled:opacity-60"
            disabled={create.isPending}
          >
            {create.isPending ? "Opening…" : "Open chamber"}
          </button>
          {create.error && (
            <p className="sm:col-span-4 text-sm text-red-400">{(create.error as Error).message}</p>
          )}
        </form>
      )}
        ...
        {chambers?.map((c) => (
          <li key={c.id} className="rounded-lg border border-border bg-secondary/30 p-5">
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs ${c.status === "open" ? "bg-emerald-500/15 text-emerald-300" : "bg-muted text-muted-foreground"}`}
              >
                {c.status}
              </span>
            </div>
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              {c.member_count}/{c.capacity} present
            </p>
            <div className="mt-4 flex gap-2">
              {c.joined ? (
                ...
                <div className="flex w-full gap-2">
                  ...
                  <button
                    ...
                    disabled={pseudo.trim().length < 2 || join.isPending}
                    className="rounded-md bg-primary px-3 py-1.5 text-sm text-primary-foreground hover:opacity-90 disabled:opacity-60"
                  >
                    Join
                  </button>
                  <button
                    onClick={() => setJoinFor(null)}
                    className="rounded-md border border-border bg-secondary px-2 py-1.5 text-xs text-muted-foreground"
                  >
                    Cancel
                  </button>
                </div>
              ...
              {isPastoral && !c.joined && (
                <Link
                  to="/chambers/$id"
                  params={{ id: c.id }}
                  className="rounded-md border border-border bg-secondary px-3 py-1.5 text-sm text-muted-foreground hover:text-ivory"
                >
                  Steward view
                </Link>
              )}
            </div>
            {join.error && joinFor === c.id && (
              <p className="mt-2 text-sm text-red-400">{(join.error as Error).message}</p>
            )}
          </li>
        ))}
        {chambers?.length === 0 && (
          <li className="text-muted-foreground">
            No chambers yet.{isPastoral ? " Open the first one above." : ""}
          </li>
        )}
      </ul>
    </main>
