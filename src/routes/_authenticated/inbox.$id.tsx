...
const STATUSES = ["received","in_review","being_prayed_for","pastor_assigned","responded","resolved"] as const;
...
function Detail() {
  ...
  const respond = useMutation({
    mutationFn: () => respondFn({
      data: { submission_id: id, body, scripture_reference: scripture || null, is_internal_note: isNote },
    }),
    onSuccess: () => { setBody(""); setScripture(""); setIsNote(false); invalidate(); },
  });

  if (isLoading) return <main className="mx-auto max-w-4xl px-4 py-10 text-muted-foreground">Loading…</main>;
  if (!data) return <main className="mx-auto max-w-4xl px-4 py-10 text-muted-foreground">Not found.</main>;
  const { submission, responses, alerts } = data;

  return (
    <main className="mx-auto max-w-4xl px-4 py-10">
      <Link to="/inbox" className="text-xs uppercase tracking-[0.3em] text-muted-foreground hover:text-ivory">← Inbox</Link>
      ...
      {submission.risk_flagged && (
        <div className="mt-4 rounded-lg border border-red-500/40 bg-red-950/30 p-4">
          ...
          <p className="mt-1 text-xs text-red-200/80">Matched: {(submission.risk_keywords ?? []).join(", ") || "—"}</p>
          {alerts.filter((a) => !a.acknowledged_at).map((a) => (
            <button key={a.id} onClick={() => ack.mutate(a.id)} disabled={ack.isPending}
      ...
      <section className="mt-6 rounded-lg border border-border bg-secondary/40 p-5">
        <p className="whitespace-pre-wrap font-serif text-lg leading-relaxed text-ivory">{submission.content}</p>
      ...
      <section className="mt-6 space-y-3">
        ...
        {responses.map((r) => (
          <article key={r.id} className={`rounded-lg border p-4 ${r.is_internal_note ? "border-amber-700/40 bg-amber-950/20" : "border-border bg-secondary/30"}`}>
            <p className="text-xs uppercase tracking-wider text-muted-foreground">
              {r.is_internal_note ? "Internal note" : "Pastoral reply"} · {r.author_display_name ?? "Pastor"} · {new Date(r.created_at).toLocaleString()}
            </p>
            <p className="mt-2 whitespace-pre-wrap font-serif text-ivory">{r.body}</p>
            {r.scripture_reference && <p className="mt-2 text-sm text-gold">— {r.scripture_reference}</p>}
          </article>
        ))}
      </section>
      ...
      <section className="mt-6 rounded-lg border border-gold/30 bg-secondary/40 p-5">
        ...
        <input value={scripture} onChange={(e) => setScripture(e.target.value)} maxLength={120}
        ...
        {respond.error && <p className="mt-2 text-sm text-red-400">{(respond.error as Error).message}</p>}
        <button onClick={() => respond.mutate()} disabled={respond.isPending || body.trim().length < 2}
