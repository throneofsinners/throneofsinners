...
function Invites() {
  ...
  if (error) return <main className="mx-auto max-w-3xl px-4 py-10 text-red-300">{(error as Error).message}</main>;
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Add an email and role. When that person signs up at <code>/auth</code> with the invited email, the role is granted automatically.
      </p>
      ...
      <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }}
        ...
        <button disabled={create.isPending} className="rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground hover:opacity-90 disabled:opacity-60">
          {create.isPending ? "Inviting…" : "Send invite"}
        </button>
        {create.error && <p className="w-full text-sm text-red-400">{(create.error as Error).message}</p>}
      </form>

      <div className="mt-6 overflow-hidden rounded-lg border border-border">
        <table className="w-full text-left text-sm">
          <thead className="bg-secondary/60 text-xs uppercase tracking-wider text-muted-foreground">
            <tr><th className="px-3 py-2">Email</th><th className="px-3 py-2">Role</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Created</th><th className="px-3 py-2"></th></tr>
          </thead>
          <tbody>
            ...
            {data?.map((i) => (
              <tr key={i.id} className="border-t border-border/60">
                ...
                <td className="px-3 py-2">{i.accepted_at ? <span className="text-emerald-300">Accepted</span> : <span className="text-muted-foreground">Pending</span>}</td>
                <td className="px-3 py-2 text-muted-foreground">{new Date(i.created_at).toLocaleDateString()}</td>
                <td className="px-3 py-2 text-right">
                  {!i.accepted_at && (
                    <button onClick={() => revoke.mutate(i.id)} className="text-xs text-red-300 hover:underline">Revoke</button>
                  )}
                </td>
              </tr>
            ))}
            {data?.length === 0 && <tr><td colSpan={5} className="px-3 py-6 text-muted-foreground">No invitations yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </main>
